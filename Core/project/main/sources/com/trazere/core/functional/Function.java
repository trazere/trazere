...
public interface Function<A, R> {
	 ...
	 * @since 2.0
	 */
	default <NA> Function<NA, R> compose(final Function<? super NA, ? extends A> function) {
	 ...
	 * @since 2.0
	 */
	default Function<A, Maybe<R>> then(final Predicate<? super R> filter) {
		assert null != filter;
		
		final Function<A, R> self = this;
		return arg -> Maybe.some(self.evaluate(arg)).filter(filter);
	}
	 ...
	 * @since 2.0
	 */
	default <TR> Function<A, TR> then(final Function<? super R, ? extends TR> function) {
		assert null != function;
