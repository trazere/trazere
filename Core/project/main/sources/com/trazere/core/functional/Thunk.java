...
public interface Thunk<T> {
	...
	/**
	 * Gets a view of this thunk that filters the value using the given filter.
	 * 
	 * @param filter Predicate to use to filter the values.
	 * @return The built function.
	 * @since 2.0
	 */
	default Thunk<Maybe<T>> filtered(final Predicate<? super T> filter) {
		assert null != filter;
		
		final Thunk<T> self = this;
		return () -> Maybe.some(self.evaluate()).filter(filter);
	}
	 ...
	 * @since 2.0
	 */
	default <R> Thunk<R> mapped(final Function<? super T, ? extends R> function) {
